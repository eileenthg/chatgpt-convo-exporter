...
(async () => {
  ...
  // batch management
  const batchsize = 200;
  const batchnum = Math.ceil(total / batchsize)
  
  
// start batch logic here

  const zipEntries = [];
  let delayCatch = DELAY;
  let tries = 0;
  const downloaded = []; // { fname, title, convo, fileMap }
  
  
    for (let offset = 0; offset < batchnum; offset++){
        
      for (let i = 0 + offset; i < total; i = i + batchnum) {
        ...
        try {
          ...
          tries++;
          if (tries < retries) {
            console.log("failed, retrying with delay: "+ delayCatch);
            await sleep(delayCatch);
            delayCatch = delayCatch + DELAY;
            i--;
          } else {
            failed++;
            tries = 0;
            delayCatch = DELAY;
            failedConvoList.push({ cid, title });
            console.log("Given up. Proceeding to next.");
          }
        }
      ...
      failedConvoList.forEach(function(entry) {
        console.log(entry.cid + " || " + entry.title);
      });
      ...
      let batchMsg = `Batch of $(batchnum)`;
      console.log("Batch " + offset + " of " + batchnum);
